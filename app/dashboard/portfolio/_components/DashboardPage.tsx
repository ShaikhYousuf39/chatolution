...
const DashboardPage = () => {
  ...
  const activeSidebarSection = (() => {
    ...
    if (pathname?.startsWith('/dashboard/portfolio/profile')) return 'Profile'
    return 'Content'
  })()
  ...
  const addFieldOption = (fieldId: string) => {
    setCustomFields((prev) =>
      prev.map((field) => {
        if (field.id !== fieldId) return field
        if (field.type !== 'checkboxes' && field.type !== 'radio') return field
        const nextLabel = `Option ${field.options.length + 1}`
        return { ...field, options: [...field.options, nextLabel] }
      })
    )
  }

  const toggleCustomFieldEditing = (fieldId: string) => {
    setEditingCustomFieldId((prev) => (prev === fieldId ? null : fieldId))
  }

  const deleteCustomField = (id: string) => {
    setCustomFields((prev) => prev.filter((field) => field.id !== id))
    setEditingCustomFieldId((prev) => (prev === id ? null : prev))
  }
              ...
              <div className="mt-7 rounded-xl shadow shadow-gray-200 bg-white p-3">
                ...
                <div className=" space-y-6">
                  ...
                  {customFields.map((field) => {
                    ...
                    const canEditOptions = field.type === 'checkboxes' || field.type === 'radio'
                    return (
                      <div key={field.id} className="mt-10">
                        <div className="flex items-center justify-between mb-4">
                          {isEditing ? (
                            <input
                              ...
                              className="w-full max-w-sm rounded-lg border border-slate-200 bg-white px-3 py-2 text-base font-medium text-slate-700 outline-none focus:ring-2 focus:ring-slate-200"
                            />
                          ) : (
                            <div className="font-medium text-xl">
                              {field.label.trim().length > 0 ? field.label : 'Select Your Preferences'}
                            </div>
                          )}
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className={`flex h-6 w-6 items-center justify-center rounded-md transition ${canEditOptions ? 'text-blue-600 hover:bg-blue-50' : 'text-slate-300 cursor-not-allowed'
                                }`}
                              title={canEditOptions ? 'Add option' : 'No options for this field'}
                            >
                            ...
                            <button
                              onClick={() => toggleCustomFieldEditing(field.id)}
                              className={`flex h-6 w-6 items-center justify-center rounded-md transition ${isEditing ? 'text-green-700 bg-green-50' : 'text-green-600 hover:bg-green-50'
                                }`}
                              title={isEditing ? 'Finish editing' : 'Edit options'}
                        ...
                        <div className="rounded-xl shadow shadow-gray-100 p-6">
                                ...
                                <div key={index} className="flex items-center gap-3">
                                  ...
                                  {isEditing ? (
                                    <input
                                      ...
                                      className="flex-1 rounded-md border border-slate-200 bg-white px-2 py-1 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-slate-200"
                                    />
                                  ) : (
...
const AddTabIcon = ({ className }: { className?: string }) => (
  <svg width="50" height="50" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg"><g clip-path="url(#clip0_754_1479)"><path d="M40.625 50H9.375C4.19688 50 0 45.8031 0 40.625V9.375C0 4.19688 4.19688 0 9.375 0H40.625C45.8031 0 50 4.19688 50 9.375V40.625C50 45.8031 45.8031 50 40.625 50Z" fill="#0F67FD" fill-opacity="0.05"></path><path d="M27.3437 12.5H15.3641C13.7813 12.5 12.5 13.7813 12.5 15.3641V31.5094C12.5 33.0938 13.7813 34.375 15.3641 34.375H22.2281C21.6656 33.1875 21.3531 31.8641 21.3531 30.4688C21.3531 29.2703 21.5828 28.125 22.0094 27.0734C21.9672 27.0844 21.9266 27.0844 21.8734 27.0844H16.6656C16.0922 27.0844 15.6234 26.6156 15.6234 26.0422C15.6234 25.4688 16.0937 25 16.6672 25H21.875C22.2703 25 22.625 25.2297 22.7922 25.5625C23.4687 24.5109 24.3437 23.6141 25.375 22.9172H16.6672C16.0937 22.9172 15.625 22.4484 15.625 21.875C15.625 21.3016 16.0937 20.8328 16.6672 20.8328H26.0422C26.6156 20.8328 27.0844 21.3016 27.0844 21.875C27.0844 21.9266 27.0844 21.9688 27.0734 22.0109C28.0422 21.6156 29.1047 21.3859 30.2094 21.3656V15.3656C30.2078 13.7812 28.9266 12.5 27.3437 12.5ZM20.8328 18.75H16.6656C16.0938 18.75 15.625 18.2813 15.625 17.7078C15.625 17.1344 16.0937 16.6656 16.6672 16.6656H20.8344C21.4078 16.6656 21.8766 17.1344 21.8766 17.7078C21.8766 18.2813 21.4062 18.75 20.8328 18.75Z" fill="#0F67FD"></path><path d="M30.4688 23.4375C26.5922 23.4375 23.4375 26.5922 23.4375 30.4688C23.4375 34.3453 26.5922 37.5 30.4688 37.5C34.3453 37.5 37.5 34.3453 37.5 30.4688C37.5 26.5922 34.3453 23.4375 30.4688 23.4375ZM33.3328 31.5109H31.5094V33.3344C31.5094 33.9094 31.0422 34.3766 30.4672 34.3766C29.8922 34.3766 29.425 33.9094 29.425 33.3344V31.5109H27.6016C27.0297 31.5109 26.5625 31.0437 26.5625 30.4688C26.5625 29.8938 27.0297 29.4266 27.6047 29.4266H29.4281V27.6031C29.4281 27.0281 29.8953 26.5609 30.4703 26.5609C31.0453 26.5609 31.5125 27.0281 31.5125 27.6031V29.4266H33.3359C33.9109 29.4266 34.3781 29.8938 34.3781 30.4688C34.3781 31.0437 33.9094 31.5109 33.3328 31.5109Z" fill="#0F67FD" fill-opacity="0.5"></path></g><defs><clipPath id="clip0_754_1479"><rect width="50" height="50" fill="white"></rect></clipPath></defs></svg>
)
